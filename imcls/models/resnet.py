...
import random
import math
from .style_hallucination import StyleHallucination

class ResNet(nn.Module):
    def __init__(self, block, layers, jigsaw_classes=1000, classes=100, **kwargs):
        self.inplanes = 64
        ...
        SHM = kwargs.get('SHM', False)
        concentration_coeff = kwargs.get('concentration_coeff', 0.)
        base_style_num = kwargs.get('base_style_num', 64)
        if SHM:
            self.shm = StyleHallucination(concentration_coeff, base_style_num)
        else:
            self.shm = None
    ...
    def forward(self, x, gt=None, flag=None, epoch=None, return_style=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        if self.shm is not None and self.training:
            x_tuple = self.shm(x)
        ...
        rc_feats = F.adaptive_avg_pool2d(x, (1, 1))
        rc_feats = rc_feats.reshape(x.size(0), -1)
...
class ResNetAutoEncoder(nn.Module):
    ...
    def forward(self, x, gt=None, flag=None, epoch=None, return_style=False):
        ...
        if self.shm is not None and not self.training:
            x_tuple = self.shm(x)
            x = x_tuple[1]
        
        return self.decoder(x)

def resnet18ae(pretrained=True, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNetAutoEncoder(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
