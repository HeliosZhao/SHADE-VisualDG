...
from mmseg.models.utils import StyleHallucination
...
class MixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        if self.shm_cfg is not None:
            self._init_style_hallucination()

    def _init_style_hallucination(self, ):
        self.use_shm = True
        self.shm_layer = self.shm_cfg.get('layer', 1)
        shm_mode = self.shm_cfg.get('mode', 'shm')

        self.shm = StyleHallucination(self.shm_cfg['concentration_coeff'], self.shm_cfg['base_style_num'], shm_mode)
    ...
    def forward_features(self, x, return_style=False, use_shm=True):
        B = x.shape[0]
        outs = []
        ...
        # ipdb.set_trace()
        if return_style and self.shm_layer == 1:
            return x
        if self.training and self.use_shm and self.shm_layer == 1 and use_shm:
            x_tuple = self.shm(x)
    ...
    def forward(self, x, return_style=False, use_shm=True):
        x = self.forward_features(x, return_style, use_shm)
        # x = self.head(x)
