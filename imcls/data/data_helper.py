...
def get_train_dataloader(args, patches):
    ...
    for dname in dataset_list:
        ...
        train_dataset = JigsawNewDataset(args.data_root, name_train, labels_train, patches=patches, img_transformer=img_transformer,
                                      tile_transformer=tile_transformer, jig_classes=30, bias_whole_image=args.bias_whole_image)
        if limit:
            train_dataset = Subset(train_dataset, limit)
        datasets.append(train_dataset)
        val_datasets.append(
            JigsawTestNewDataset(args.data_root, name_val, labels_val, img_transformer=get_val_transformer(args),
...
def get_val_dataloader(args, patches=False):
    ...
    val_dataset = JigsawTestNewDataset(args.data_root, names, labels, patches=patches, img_transformer=img_tr, jig_classes=30)
...
def get_val_list_dataloader(args, patches=False):
    dataset_list = args.target
    assert isinstance(dataset_list, list)
    val_datasets = []
    limit = args.limit_source
    for dname in dataset_list:
        name_val, labels_val = _dataset_info(join(dirname(__file__), 'correct_txt_lists', '%s_test_kfold.txt' % dname))
