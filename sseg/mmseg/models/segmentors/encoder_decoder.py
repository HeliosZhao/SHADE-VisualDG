...
class EncoderDecoder(BaseSegmentor):
    ...
    def extract_feat(self, img, use_shm=True):
        """Extract features from images."""
        x = self.backbone(img, use_shm=use_shm)
        if self.with_neck:
            x = self.neck(x)
        return x

    def extract_style_feat(self, img):
        """Extract features from images."""
        x = self.backbone(img, return_style=True)
        if self.with_neck:
            x = self.neck(x)
        return x
    ...
    def forward_train(self,
        ...
        """
        # ipdb.set_trace()
        x = self.extract_feat(img, use_shm)
