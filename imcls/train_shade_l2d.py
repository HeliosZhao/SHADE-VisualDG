...
from models.resnet_l2d import resnet18
...
def get_args():
    ...
    parser.add_argument("--adv_lr", type=float, default=0.0, help="Learning rate for advstyle")
    ...
    ## shade configs
    parser.add_argument('--SHM', action='store_true', default=True,
                        help='use SHM')
    ...
    parser.add_argument('--use_augnet', action='store_true', default=False,
                        help='use augnet in shm proto')
    ...
    parser.add_argument('--rc_weight', type=float, default=1.0,
...
class Trainer:
    ...
    def retrospective_consistency(self, x, feats):
        ...
        with torch.no_grad():
            _, teacher_outs = self.teacher_model(x)
    ...
    def _SHM_init(self, epoch=None):
        ...
        for _ in range(self.args.proto_trials):
            for it, ((data, _, class_l), idx) in enumerate(self.source_loader): #enumerate(tqdm(self.source_loader)):
                data, class_l = data.to(self.device), class_l.to(self.device)

                if self.args.use_augnet:
                    inputs_max = tran(torch.sigmoid(self.convertor(data)))
                    inputs_max = inputs_max * 0.6 + data * 0.4
                    data = torch.cat([inputs_max, data])

                features = self.extractor(data, return_style=True)
        ...
        self.extractor.shm.proto_mean.copy_(proto_mean)
        self.extractor.shm.proto_std.copy_(proto_std)
    ...
    def _do_epoch(self, epoch=None):
        ...
        tran = transforms.Normalize([0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        for it, ((data, _, class_l), idx) in enumerate(self.source_loader):
            data, class_l = data.to(self.device), class_l.to(self.device)
            ...
            ## SHM double the batch
            labels = torch.cat((labels, labels))
            logits, tuple = self.extractor(data_aug)

            rc_feats = tuple['rc_feats']
            ## mi loss only use original samples
            tuple['Embedding'] = tuple['Embedding'][:2*class_l.size(0)]
            tuple['mu'] = tuple['mu'][:2*class_l.size(0)]
            tuple['logvar'] = tuple['logvar'][:2*class_l.size(0)]
            ...
            loss_con_style = self.style_consistency(logits)
            loss_con_retro = self.retrospective_consistency(data_aug, rc_feats)

            loss = loss + loss_con_style + loss_con_retro

            # if self.args.adv_lr > 0:
            #     class_loss_max = criterion(logits_max, class_l)
            #     loss += class_loss_max
            ...
            # STAGE 2 update augnet
            inputs_max =tran(torch.sigmoid(self.convertor(data, estimation=True)))
            inputs_max = inputs_max * 0.6 + data * 0.4
            data_aug = torch.cat([inputs_max, data])

            # forward with the adapted parameters
            outputs, tuples = self.extractor(x=data_aug, use_shm=False)
            ...
            #                  },
            #                 {"class": torch.sum(cls_pred[class_l.size(0):2*class_l.size(0)] == class_l.data).item()},
            #                 class_l.shape[0])
            ...
            if len(self.args.target) > 1:
                ...
                for i, loader in enumerate(self.target_loader):
                    ...
                    # if not self.args.no_verbose:
                        # print('Epoch {} \t {} \t Acc {:.4f}'.format(epoch, i, class_acc))

                    avg_acc += class_acc
                avg_acc = avg_acc / len(self.args.target)
                if not self.args.no_verbose:
                    print('Epoch {} \t Acc {:.4f}'.format(epoch, avg_acc))
    ...
    def do_eval(self):
        ...
        with torch.no_grad():
            for phase, loader in self.test_loaders.items():
                ...
                print('{} \t Acc {:.4f}'.format(phase, class_acc))
...
def main():
    ...
    if args.task == 'PACS':
        ...
        # args.source = ['art_painting']
        # args.target = ['photo', 'cartoon', 'sketch']

        # args.source = ['cartoon']
        # args.target = ['art_painting', 'photo', 'sketch']

        # args.source = ['sketch']
        # args.target = ['art_painting', 'photo', 'cartoon']
